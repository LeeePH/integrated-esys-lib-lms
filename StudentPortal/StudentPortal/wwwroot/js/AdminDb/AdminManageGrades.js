...
/* ---------- fixed columns ---------- */
const FIXED_COLS = ['Student_ID', 'Last_Name', 'First_Name', 'Middle_Name', 'Standing'];
...
function handleFile(file) {
    ...
    reader.onload = evt => {
        ...
        const missing = FIXED_COLS.filter(c => !fixedIndices[c]);
        importDbBtn.disabled = missing.length > 0;
...
function applySearch(q) {
    ...
    const lastKey = fixedIndices['Last_Name'];

    filteredData = parsedData.filter(r => {
        if (sidKey && String(r[sidKey]).toLowerCase().includes(q)) return true;
        if (lastKey && String(r[lastKey]).toLowerCase().includes(q)) return true;
...
async function sendToServer() {
    const missing = FIXED_COLS.filter(c => !fixedIndices[c]);
    if (missing.length) return showToast("Cannot import: missing required columns");

    const rows = (filteredData.length ? filteredData : parsedData).map(r => {
        const o = {};
        FIXED_COLS.forEach(c => {
            const key = fixedIndices[c];
            o[c] = key ? r[key] : "";
        });
        return o;
    });
