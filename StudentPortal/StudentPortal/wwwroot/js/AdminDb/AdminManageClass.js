...
// ---------------- DUMMY ATTENDANCE COLLECTION ----------------
// This is used when there is no real data in the database.
// Each export will add a new Attendance column based on this collection.
...
// Keeps track of each export's attendance column so subsequent exports
// include previous columns as well.
let attendanceSnapshots = []; // [{ label: string, statusById: { [id]: 'Present'|'Late'|'Absent' } }]
let exportCounter = 0;
const ATTENDANCE_ENUM = ["Present", "Late", "Absent"];
    ...
    if (arr.length === 0) {
        ...
        arr.forEach(student => {
            ...
            card.innerHTML = `
                ...
                <div class="actions">
                    <button class="mark-present" data-id="${student.id}" data-status="Present">✅</button>
                    <button class="mark-absent" data-id="${student.id}" data-status="Absent">❌</button>
                </div>`;
...
function exportData() {
    try {
        ...
        // 1) Generate a new attendance snapshot for this export
        exportCounter += 1;
        const label = `Attendance_${exportCounter}`;
        const statusById = {};

        attendance_data.forEach((s, idx) => {
            // Cycle deterministically through Present/Late/Absent so export is reproducible
            const status = ATTENDANCE_ENUM[(exportCounter + idx) % ATTENDANCE_ENUM.length];
            statusById[s.student_id] = status;
        });

        attendanceSnapshots.push({ label, statusById });

        // 2) Build header row
        const header = ["Student_ID", "Last_Name", "First_Name", "Middle_Name"];
        attendanceSnapshots.forEach(snap => header.push(snap.label));
        header.push("Task", "Assessment", "Standing");

        // 3) Build data rows from dummy collection
        const rows = [header];
        attendance_data.forEach(student => {
            const taskAttained = Number(student.task_attained || 0);
            const taskTotal = Number(student.task_total || 0);

            // Attendance values for each snapshot in order
            const attStatuses = attendanceSnapshots.map(snap => snap.statusById[student.student_id] || "Absent");

            // Compute standing using our grading system
            const { standing } = computeStanding(attStatuses, taskAttained, taskTotal);

            // Task column shown as "attained/total"
            const taskDisplay = taskTotal > 0 ? `${taskAttained}/${taskTotal}` : "";
            ...
            row.push(taskDisplay, assessmentDisplay, standing);

            rows.push(row);
        });

        // 4) Use SheetJS (XLSX) to create an Excel file from the rows
        ...
        const fileName = `Class_${classCode}_attendance_export.xlsx`;
        XLSX.writeFile(wb, fileName);

        if (typeof window.showToast === 'function') window.showToast("✅ Exported dummy attendance data to Excel.");
    } catch (err) {
        console.error(err);
        if (typeof window.showToast === 'function') window.showToast("⚠️ Export failed.");
    }
}
