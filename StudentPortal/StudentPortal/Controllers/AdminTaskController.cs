...
namespace StudentPortal.Controllers
{
    public class AdminTaskController : Controller
    {
        ...
        public async Task<IActionResult> GradeSubmission(string submissionId, string grade, string feedback, bool? approve, bool? pass)
        {
            ...
            double gnum; var hasPassed = pass ?? (double.TryParse(grade, out gnum) ? gnum >= 75 : false);
            var ok = await _mongoDb.UpdateSubmissionStatusAsync(submissionId, isApproved, hasPassed, grade ?? string.Empty, feedback ?? string.Empty);
